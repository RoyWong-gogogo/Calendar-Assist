...
- **日历脚本要联网**：本机沙箱放行网络（2026-09-13 修复后实测：沙箱内直接调用中国区 Graph 返回真实日程），日历脚本在沙箱内直接运行即可，不需要升级授权；不要为了「省授权」去改代码或换数据源。
- **推送走 SSH + gh**：`origin` 已指向私有仓库 `git@github.com:RoyWong-gogogo/Calendar-Assist.git`，提交后直接 `git push`（写 .git 需要升级授权）。`gh` 的 token 存在 Windows 凭据管理器（keyring），**沙箱内的 gh 读不到 keyring**：`gh auth status` 会误报 "The token in default is invalid"、`gh api` 报 401（私有仓库端点报 404）——这是沙箱限制，不代表授权失效；只有带升级授权跑 `gh auth status` 才能看到真实状态（正常显示 `Logged in to github.com account ... (keyring)`）。gh API 操作一律直接带升级请求执行，不要因为沙箱内的报错就去重新授权。需要重新授权时执行 `gh auth login --hostname github.com --git-protocol ssh --skip-ssh-key --web`（沙箱内同样需要升级授权；前置 `$env:GH_BROWSER='echo'` 可让它只打印地址、不自动弹窗），把一次性代码与 https://github.com/login/device 清楚转达用户并等其完成。仓库含 `data/contacts.csv` 中的同事邮箱，保持私有，改公开前必须征得用户同意。
